...
setClass("poissonInitialData",
         representation(grp1.data="poissonOverDispersedArm",
...
setClass("poissonRates",
         representation(grp1.rate="numeric",
                        grp2.rate="numeric"),
         validity=function(object) {
           r1 <- object@grp1.rate
           r2 <- object@grp2.rate
...
setClass("overdispersedPoissonStudy",
         contains="twoArm",
         representation(grp1.overdispersion="numeric",
...
setMethod("sampleLater",
          ...
          function(posteriorSample, laterStudy) {
            ...
            grp1.count <- rpois(nsim,
                                posteriorSample@grp1.rate * duration$grp1)
            grp2.count <- rpois(nsim,
...
setClass("negativeBinomialStudy",
         contains="twoArm",
         representation(overdispersion="numeric",
                        duration="numeric"),
         validity=function(object) {
...
setClass("negativeBinomialLater",
         representation(rates="poissonRates",
...
setMethod("testLater",
          ...
          function(laterSample) {
            ...
            rates <- rbind(laterSample@rates@grp1.rate,
                           laterSample@rates@grp2.rate)
            flag <- c(rep(0, study.def@size@grp1),
            ...
            apply(rates, 2,
                  function(column, dispersion, flag,
                    ...
                    fuzz.2 <- rgamma(grp2.size,
                                     shape=dispersion,
                                     rate=dispersion)
                    count.1 <- rpois(grp1.size,
                                     fuzz.1 * column[[1]] * duration)
                    count.2 <- rpois(grp2.size,
