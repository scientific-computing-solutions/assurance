...
setClass( "generalLaterStudy",
          representation( required.success="numeric",
                          studies="list",
                          at.least="logical" )
          )
...
setClass( "generalLaterSample",
          representation( required.success="numeric",
                          results="list",
                          at.least="logical" ))

setMethod("sampleLater",
          signature( posteriorSample="ANY", 
          ...
          function( posteriorSample, laterStudy ) {
              new( "generalLaterSample",
                    ...
                    results=lapply( laterStudy@studies,
                      function(x) { sampleLater(posteriorSample, x) } ),
...
pts.rec = function( k, n, p ) {
 ...
 # Special case 
 if( k==0 ) return( prod( q[1:n] ))
 
 # Base case
 if( k == 1 & n == 1 ) return( p[1] ) 
 
 # Case 1: k = n
 if( k == n ) return( prod( p[1:n] ) )
...
setClass("twoArm",
         representation(size="twoWayStudySize",
...
setClass("oneArmResponseRate",
         representation(size="oneWayStudySize",
                        rate="numeric"),
         validity=function(object) {
           rate <- object@rate
           if (length(rate) != 1) {
